...
import ServerNpcharacterReaderStruct from '~/structs/server/npcharacter/reader_struct';
...
export default function* defaultSaga({ projectId, actions, fileData }) {
  ...
  while (readerStruct.length > i) {
    ...
    fillBuffer(
      ...
      object => ({
        nIndex: object.nIndex,
        strCode: get(object.npcharacter, 'strCode', ''),
        strKo: '-',
        strBr: '-',
        strCh: '-',
        strEn: '-',
        ...
        strTa: '-',
        strVi: '-',
        strGlobal: get(object.npcharacter, 'strName', ''),
      }),
    );
